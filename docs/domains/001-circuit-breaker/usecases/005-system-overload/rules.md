...
| BR-042a-probe-join-timeout | 探针退出超时可配置 | 探针线程退出等待时间（startProbe/stopProbe）通过系统属性 `circuitbreaker.probeJoinTimeoutMs` 可配置；默认 2000ms，范围 [0, 60000]；超时后继续退出但不再等待 | UC-007 | **P4 修复**（对抗性审查） |
...
| 1.1 | 2026-08-05 | 新增 BR-042a（探针退出超时可配置） | P4：硬编码 join(2000) 改为系统属性可配置 | 代码审查 |
