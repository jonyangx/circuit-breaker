...
| `version` | int | 配置版本号（低 6 位进入 token） |
...
| 36–59 | 24 | lastUpdateMs | 上次更新相对 ms；Δt=(now−last)&0xFFFFFF |
...
| 58–61 | 4 | generation | 权威代际，迁移 +1（循环 16） |
| 0–57 | 58 | endTimeMs | OPEN 结束的绝对 ms |

## 7. 关系与不变量

- `version`（token 低 6 位）= acquire 命中的 `CONFIGS[resourceId].version`；release 时校验换代。
