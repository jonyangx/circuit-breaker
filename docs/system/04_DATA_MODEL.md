...
9 个 `public final` 字段：`mask`(int) · `qps`(long) · `capacity`(long) · `errThresholdPpm`(int,0..1e6) · `minCalls`(int) · `openMillis`(long) · `ewmaTauMs`(long) · `concurrencyLimit`(int) · `version`(int)。整体替换（RCU）；version 低 10 位进 token。
...
`ewmaErrorRatePpm()`：低 20 位 ppm 只读访问器。三个热点 `AtomicLong` 已 `@Contended` 填充（`concurrency[16]` 未填充，已 16 路分段竞争低）。
...
- 位宽：MASK=12 / BUCKET=4 / VERSION=10 / TIME=37；SHIFT：MASK=0/BUCKET=12/VERSION=16/TIME=26。
...
| 36–55 | 20 | lastUpdateMs | Δt=(now−last)&0xFFFFF |
...
| 54–61 | 8 | generation | 权威代际，迁移 +1（循环 256）；HALF_OPEN 时 endTime 兼作探路截止 |
| 0–53 | 54 | endTimeMs | OPEN 结束绝对 ms / HALF_OPEN 探路截止 |

## 7. bucketState 位布局（令牌桶）
高 42 位 Time_last（≈139 年） \| 低 22 位 Tokens（≤4,194,303）。refill `add=(now−tLast)*qps/1000`，nTok cap TOKEN_MASK。

## 8. 关系与不变量
- token.version == acquire 时 CONFIGS.version（低 10 位）；release 校验换代。
